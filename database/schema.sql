...
CREATE TABLE IF NOT EXISTS `turmas` (
    ...
    `razao_social` VARCHAR(255) NULL COMMENT 'Razão Social do Tomador do Serviço',
    `cnpj_tomador` VARCHAR(20) NULL COMMENT 'CNPJ ou CPF do Tomador do Serviço',
    `cidade_uf` VARCHAR(100) NULL COMMENT 'Município e UF de Faturamento / Prestação',
    `email_financeiro` VARCHAR(255) NULL COMMENT 'E-mail para envio de NFS-e e cobrança',
    `numero_os_contrato` VARCHAR(50) NULL COMMENT 'Número da Ordem de Serviço ou Contrato',
    ...
    `valor_unitario` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT 'Valor unitário da hora-aula, aluno ou base',
    ...
    `portal_certificados_modo` ENUM('nenhum', 'coordenacao', 'download_direto') NOT NULL DEFAULT 'nenhum' COMMENT 'Modo de entrega de certificados no portal do aluno',
...
CREATE TABLE IF NOT EXISTS `materiais_turma` (
    ...
    `ativo` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=Visível para alunos, 0=Oculto/Rascunho',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
